...
do {									\
	lock_acquire_exclusive(l, s, t, NULL, i);			\
	if (t)								\
		dept_rwsem_write_trylock(&(l)->dmap, "rwsem_write_unlock", i);\
	else								\
		dept_rwsem_write_lock(&(l)->dmap, s, "rwsem_write_unlock", i);\
} while (0)
...
do {									\
	...
	if (t)								\
		...
		dept_rwsem_write_lock_nest(&(l)->dmap, &(n)->dmap, i);	\
	else								\
		dept_rwsem_write_lock(&(l)->dmap, s, "rwsem_write_unlock", i);\
} while (0)
...
do {									\
	lock_acquire_shared(l, s, t, NULL, i);				\
	if (t)								\
		dept_rwsem_read_trylock(&(l)->dmap, "rwsem_read_unlock", i);\
	else								\
		dept_rwsem_read_lock(&(l)->dmap, s, "rwsem_read_unlock", i);\
} while (0)
